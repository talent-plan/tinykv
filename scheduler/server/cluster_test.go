...
func (s *baseCluster) getClusterConfig(c *C, clusterID uint64) *metapb.Cluster {
	req := &schedulerpb.GetClusterConfigRequest{
		Header: testutil.NewRequestHeader(clusterID),
	}

	resp, err := s.grpcSchedulerClient.GetClusterConfig(context.Background(), req)
	c.Assert(err, IsNil)
...
func (s *testClusterSuite) TestGetPutConfig(c *C) {
	...
	req := &schedulerpb.PutClusterConfigRequest{
		Header: testutil.NewRequestHeader(clusterID),
	...
	resp, err := s.grpcSchedulerClient.PutClusterConfig(context.Background(), req)
	c.Assert(err, IsNil)
	c.Assert(resp, NotNil)
