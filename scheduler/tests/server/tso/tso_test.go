...
func (s *testTimeFallBackSuite) SetUpSuite(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/pingcap-incubator/tinykv/scheduler/server/tso/fallBackSync", `return(true)`), IsNil)
	c.Assert(failpoint.Enable("github.com/pingcap-incubator/tinykv/scheduler/server/tso/fallBackUpdate", `return(true)`), IsNil)
	var err error
	...
	svr.Close()
	failpoint.Disable("github.com/pingcap-incubator/tinykv/scheduler/server/tso/fallBackSync")
	failpoint.Disable("github.com/pingcap-incubator/tinykv/scheduler/server/tso/fallBackUpdate")
	err = svr.Run(s.ctx)
	c.Assert(err, IsNil)
	s.cluster.WaitLeader()
}
