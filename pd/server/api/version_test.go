...
func (s *testVersionSuite) TestGetVersion(c *C) {
	...
	go func(cfg *config.Config) {
		...
		c.Assert(err, IsNil)
		c.Assert(failpoint.Enable("github.com/pingcap-incubator/tinykv/pd/server/memberNil", `return(true)`), IsNil)
		reqCh <- struct{}{}
		err = s.Run(ctx)
		c.Assert(err, IsNil)
		ch <- s
	...
	c.Assert(failpoint.Disable("github.com/pingcap-incubator/tinykv/pd/server/memberNil"), IsNil)
}
