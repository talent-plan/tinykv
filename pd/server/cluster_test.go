...
func (s *testClusterSuite) TestRaftClusterMultipleRestart(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/pingcap-incubator/tinykv/pd/server/highFrequencyClusterJobs", `return(true)`), IsNil)
	for i := 0; i < 100; i++ {
		err = s.svr.createRaftCluster()
		c.Assert(err, IsNil)
...
func (s *testClusterSuite) TestStoreVersionChange(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/pingcap-incubator/tinykv/pd/server/versionChangeConcurrency", `return(true)`), IsNil)
	wg.Add(1)
	go func() {
		defer wg.Done()
	...
	c.Assert(failpoint.Disable("github.com/pingcap-incubator/tinykv/pd/server/versionChangeConcurrency"), IsNil)
}
