...
func TestNodeStep(t *testing.T) {
	for i, msgn := range pb.MessageType_name {
		...
		n.Step(context.TODO(), pb.Message{MsgType: msgt})
		// Proposal goes to proc chan. Others go to recvc chan.
		if msgt == pb.MessageType_MsgPropose {
			select {
			case <-n.propc:
			default:
...
func TestNodeStepUnblock(t *testing.T) {
	...
	for i, tt := range tests {
		...
		go func() {
			err := n.Step(ctx, pb.Message{MsgType: pb.MessageType_MsgPropose})
			errc <- err
		}()
		tt.unblock()
		select {
...
func TestNodePropose(t *testing.T) {
	...
	if msgs[0].MsgType != pb.MessageType_MsgPropose {
		t.Errorf("msg type = %d, want %d", msgs[0].MsgType, pb.MessageType_MsgPropose)
	}
...
func TestNodeProposeWaitDropped(t *testing.T) {
	...
	dropStep := func(r *Raft, m pb.Message) error {
		if m.MsgType == pb.MessageType_MsgPropose && strings.Contains(m.String(), string(droppingMsg)) {
...
func TestReadyContainUpdates(t *testing.T) {
	tests := []struct {
		...
		{Ready{HardState: pb.HardState{Vote: 1}}, true},
		{Ready{Entries: make([]pb.Entry, 1)}, true},
		{Ready{CommittedEntries: make([]pb.Entry, 1)}, true},
		{Ready{Messages: make([]pb.Message, 1)}, true},
		{Ready{Snapshot: pb.Snapshot{Metadata: &pb.SnapshotMetadata{Index: 1}}}, true},
	}
...
func TestAppendPagination(t *testing.T) {
	...
	n.msgHook = func(m pb.Message) bool {
	...
	for i := 0; i < 5; i++ {
		n.send(pb.Message{From: 1, To: 1, MsgType: pb.MessageType_MsgPropose, Entries: []*pb.Entry{{Data: blob}}})
	}
	n.recover()

	// After the partition recovers, tick the clock to wake everything
	// back up and send the messages.
	n.send(pb.Message{From: 1, To: 1, MsgType: pb.MessageType_MsgBeat})
	if !seenFullMessage {
